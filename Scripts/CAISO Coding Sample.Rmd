...
#Conclude
  write.csv(caiso_df, "~/Documents/Job Search/Code Sample/CAISO//Data/Actual Hourly Integrated Load by Region.csv",row.names = FALSE) #Save Data
...
#Conclude 
  ...
  keep_list <- c(keep_list, "degree_days")
...
#Conclude  
  keep_list <- c(keep_list, "day_dummies")
  rm(list = ls()[!ls() %in% keep_list]) #Tidy Up Environment   
```
...
#Here, we want to retain houly variation, but correct for weather and holidays
  ...
  plot(electric_demand_daily$date, electrical_demand_daily$MW, type = "l")
...
hourly_model_1 <- lm(MW ~ factor(hour_f)*heat + 
                         factor(hour_f)*cool + factor(hour_f)*holiday + factor(hour_f)*day_of_week + structural_break,
              data = electric_demand_hour, 
              na.action = na.exclude)
daily_model_1 <- lm()
